// crates/mid-math/src/swizzle.rs
//! Component-permutation traits for this crate's Vec2/Vec3/Vec4-family types:
//! read-only getters (`.xy()`, `.xzy()`, `.wzyx()`, ...) covering every
//! same-width, narrowing, and widening permutation, plus same-or-narrower-width
//! replacement setters (`.with_xy(rhs)`, `.with_xyz(rhs)`, ...) that return a
//! new value with the named components replaced from `rhs` and everything
//! else left as-is. No `with_` setter widens (e.g. there is no 4-letter
//! `with_` on `Vec4Swizzles`) since replacing every component isn't a partial
//! update — just build a new value with `::new()` instead.
//!
//! `Vec2Swizzles`/`Vec3Swizzles`/`Vec4Swizzles` are implemented per concrete
//! type via the `impl_vec{2,3,4}_swizzle!` macros below, the same pattern
//! `deref.rs` already uses for this crate's backend-split types: every
//! backend's `Vec3`/`Vec4` already exposes `.x`/`.y`/`.z`/`.w` (directly, or
//! via `Deref` into `XYZ<T>`/`XYZW<T>`, see `crate::deref`) and its own
//! `new()`, so one macro invoked once per backend file is enough — no
//! per-backend generated file needed the way glam's Tera-template pipeline
//! produces one.
//!
//! Perf note: every getter here goes through `<Output>::new(...)` — none of
//! them use a single in-register SIMD shuffle instruction the way glam's
//! SSE2/NEON/WASM/coresimd backends do for same-width swizzles. That's a
//! real, well-defined follow-up (see docs/mid-math.md), deliberately
//! deferred: it needs verifying each backend's shuffle-immediate encoding
//! *and* its padding-lane behavior (`Vec3`'s 4th lane) with no compiler
//! available to catch a wrong lane, across several hundred call sites per
//! backend. `Self::new(...)` reuses each backend's own already-correct
//! constructor instead, so padding is handled exactly as it already is
