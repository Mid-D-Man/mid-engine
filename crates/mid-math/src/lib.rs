...
//!
//! Storage strategy:
//! - Vec3 / Vec4 / Quat : 16-byte aligned (SSE2-friendly on the 2010 Mac)
//! - Vec2               : 8 bytes, tight packing for UV / 2D physics
//! - Mat3               : 36 bytes column-major (tight, no alignment padding)
//! - Mat4               : 64 bytes column-major, 16-byte aligned columns
...
/// Epsilon used for approximate float comparisons throughout the library.
...
/// Inverse lerp: what `t` would produce `v` between `a` and `b`?
#[inline] pub fn inverse_lerp(a: f32, b: f32, v: f32) -> f32 {
...
/// Clamp `v` to `[min, max]`.
#[inline] pub fn clamp(v: f32, min: f32, max: f32) -> f32 { v.clamp(min, max) }

/// Clamp `v` to `[0, 1]`.
#[inline] pub fn saturate(v: f32) -> f32 { v.clamp(0.0, 1.0) }

/// Degrees → radians.
#[inline] pub fn to_radians(deg: f32) -> f32 { deg * DEG2RAD }

/// Radians → degrees.
#[inline] pub fn to_degrees(rad: f32) -> f32 { rad * RAD2DEG }

/// Float approximate equality using `EPSILON`.
#[inline] pub fn approx_eq(a: f32, b: f32) -> bool { (a - b).abs() < EPSILON }
