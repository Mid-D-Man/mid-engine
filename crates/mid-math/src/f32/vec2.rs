...
impl Vec2 {
    ...
    /// Signed 2D cross product (perp-dot): `self.x*rhs.y - self.y*rhs.x`.
    ...
    /// Build from polar coordinates `(radius, angle)`.
    #[inline(always)]
    ...
    /// Clamp the length to at most `max`.
    #[inline(always)]
    ...
    /// Clamp the length to at least `min`.
    #[inline(always)]
