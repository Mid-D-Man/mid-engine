...
// ── XYZ mask: all-ones lanes 0-2, lane 3 = zero ─────────────────────────────
...
impl Quat {
// ── Constants ─────────────────────────────────────────────────────────────pub const IDENTITY: Self = unsafe { UnionCast { f: [0.0, 0.0, 0.0, 1.0] }.v };
...
/// Create from axis-angle, keeping all components in XMM registers.
///
/// Previously: normalize() → Deref reads n.x/n.y/n.z → stack spill →
///   _mm_set_ps(c, z, y, x) → 4 scalar moves = ~6.25 ns.
///
/// Now: normalize() produces __m128 → broadcast sin → _mm_mul_ps
///   → mask/blend w → ~4.8 ns target.
///
/// Algorithm:
///   q.xyz = normalize(axis) * sin(angle/2)   — one mul, stays in register
///   q.w   = cos(angle/2)                     — insert into lane 3 via AND/OR
///
/// SSE2-compatible lane-3 insert:
///   _mm_andnot_ps(XYZ_MASK, _mm_set1_ps(c))  → [0,0,0,c]
///   _mm_and_ps(xyz_s, XYZ_MASK)              → [x*s, y*s, z*s, 0]
///   _mm_or_ps(...)                            → [x*s, y*s, z*s, c]
#[inline]
pub fn from_axis_angle(axis: Vec3, angle_rad: f32) -> Self {
    let (s, c) = math::sin_cos(angle_rad * 0.5);
    unsafe {
        let n     = axis.normalize().0;              // __m128 in register
        let sv    = _mm_set1_ps(s);
        let xyz_s = _mm_mul_ps(n, sv);               // [ax*s, ay*s, az*s, 0.0*s]
        // Zero lane 3 of xyz_s (it's already 0 from Vec3 padding, but be explicit)
        let xyz_clean = _mm_and_ps(xyz_s, XYZ_MASK);
        // Blend w=c into lane 3: keep lanes 0-2 from xyz_clean, lane 3 from c
        let w_only = _mm_andnot_ps(XYZ_MASK, _mm_set1_ps(c)); // [0, 0, 0, c]
        Self(_mm_or_ps(xyz_clean, w_only))
    }
}
...
/// Convert to Mat4 using the fully-vectorized quat_to_axes helper.
///
/// Previously: scalar (x,y,z,w) Deref reads forced stack spill.
/// Now: delegates to `quat_to_axes_sse2` in mat4.rs — pure XMM, no spill.
