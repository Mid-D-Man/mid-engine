...
//! for `f32x4` (via `LaneShuffle4`, see `wide_lane_engine.rs`). No `QuatX4` —
//! see `wide_axis_engine.rs`'s doc comment for why. Each `#[cfg(...)]` here is
//! copied from that backend's own module declaration in `wide/float/mod.rs`.
...
// ── AVX2 (additive — Vec3x8 has no scalar/neon/wasm equivalent) ─────────────
