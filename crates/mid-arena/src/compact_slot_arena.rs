...
//! # Growth
//!
//! Grows by one slot per insert past whatever's currently free, same
//! as [`SlotArena`](crate::SlotArena) — see that module's own doc
//! comment on growth for a real technique from faster crates that was
//! tried here too (ported to this file's union storage, same as every
//! other change in `SlotArena` was), confirmed on real CI to regress
//! rather than help, and reverted in full. Not re-explained twice.
