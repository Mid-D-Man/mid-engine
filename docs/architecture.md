...
| mid-ecs | Data-oriented ECS (SoA), network sync baked in from day one | **In progress.** `World` is real — `spawn`/`despawn`/`is_alive` plus a fully-wired Sparse Shell (`insert`/`get`/`get_mut`/`remove`/`has`, any `T: 'static` as a component, dense `ComponentId` not `TypeId`-hashed — grounded in Bevy ECS's real source). 66/66 real tests across `mid-collections` + `mid-ecs`. `rayon` gated to non-wasm32 targets only (`docs/mid-ecs.md`'s Platform section) — confirmed via `cargo tree`, real CI now checks the wasm32 build too. `Archetype`/`Query`/`sync`/`ffi` still skeleton — no dense/table storage for stable components yet, and no query/iteration API over the Sparse Shell yet either. Next: `World::query`-style iteration, then the Archetype Core |
