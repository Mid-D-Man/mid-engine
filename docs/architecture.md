...
| **mid-net** | Reliable UDP-class transport over QUIC (native `quinn`, browser WebTransport), pluggable `Transport` trait, hand-rolled wire codec | **In progress.** Restructured into subfolder crates this pass (`mid-net-wire`, `mid-net-transport`, `mid-net-reliable`, facade `mid-net` — see `docs/mid-net.md` "Crate Structure"), mirroring naia's own `socket/{client,server,shared}` split. All real and tested (46 tests), verified as an actual multi-crate workspace. `PlayerEvent` reliability now rides a QUIC stream, not `mid-net-reliable`'s own retransmit buffer — see `docs/mid-net.md`. Real `quinn`/`web-transport-wasm` backends (planned as `mid-net-transport-quinn`/`mid-net-transport-wasm`) and `ffi.rs` still unbuilt |
