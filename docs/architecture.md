...
| mid-ecs | Data-oriented ECS (SoA), network sync baked in from day one | **In progress.** `World`, Sparse Shell, `query`/`query2`, and a real Archetype Core with full dynamic migration — grounded in Bevy ECS's real source, cloned and read directly. FFI now built incrementally, not saved for the end (`docs/mid-ecs.md`'s FFI section) — `World` lifecycle (`spawn`/`despawn`/`is_alive`/`entity_count`) is real, verified against actual compiled C (real gcc, real `.so` and `.a`, 19/19), CI-checked every run. 111/111 real tests across `mid-collections` + `mid-ecs`. `rayon` gated to non-wasm32 targets, reconfirmed clean after every addition since. `sync`/`ffi`-for-component-data still open. Next: the FFI-span design for reading component data across the boundary, or archetype-based query iteration |
| **mid-collections** | Sparse set, generational arena, lock-free ring buffer, intrusive lists, hierarchical bitset, `std::hive`, FFI wrapper | **Two pieces built.** Real crate (`crates/mid-collections`), zero regular dependencies, `#![no_std]` + `alloc`. `SparseSet<I: SparseSetIndex, T>` — 18 tests, and now real `cargo bench` numbers vs `std::HashMap`: 2.25–5.3x faster on insert/remove, 5–25x faster on get/iterate, at every scale from 100 to 100,000 elements (see `docs/mid-collections.md`). `GenerationalIndexAllocator`/`GenerationalIndex` — 16 tests, design verified against real `slotmap` 1.0.7 source, wired into `mid-ecs`'s `World` for real. 34/34 tests total, still real rustc-1.75-locally-testable (the criterion dev-dependency added for benching does mean `cargo test -p mid-collections` alone now needs it temporarily stripped locally first — real regression, documented in root `Cargo.toml`'s comments, not hidden). Everything else in this crate stays design-doc-only, pulled in piece-by-piece exactly as `mid-ecs` needs it, per that doc |
| **mid-arena** | Arena/slot allocators, built from a real survey of 28 Rust arena crates + 3 established C arena libraries | **First piece built.** Real crate (`crates/mid-arena`), zero regular dependencies, `#![no_std]` + `alloc`. `SlotArena<T>`/`ArenaKey` — generational, value-storing, directly extends `mid-collections`' `GenerationalIndexAllocator` algorithm to actually own a `T` per slot — 16 tests, real and passing on this project's rustc-1.75 floor (same "criterion dev-dep added, `cargo test -p mid-arena` alone now needs it stripped locally first" regression `mid-collections` already carries, same reason, documented in root `Cargo.toml`). Survey: 22/28 Rust crates confirmed buildable on rustc 1.75 (real, isolated `cargo build` per crate), 10 actually benched (`std::time::Instant`, N=100,000) spanning every distinct storage approach in the survey; 3 C arena libraries (tsoding/arena.h, APR pools, talloc_pool) compiled `-O3 -march=native` and benched for real in the same pass. `compact`/`bump`/`intern`/`concurrent`/`ffi` feature gates planned, not built; GC-based approaches explicitly ruled out (collection pauses conflict with this doc's 128 Hz/60 Hz budgets). See `docs/mid-arena.md` for the full comparison and every number behind this row |
| **mid-alloc** | Composable allocator strategies (stack/pool/fallback/segregator/tracking), split out from mid-arena on purpose — different scope, not a subset | **First piece built.** Real crate (`crates/mid-alloc`), zero regular dependencies, `#![no_std]` + `alloc`. `StackAllocator`/`StackMarker` — fixed-capacity marker/rewind bump allocator for per-frame scratch storage, modeled on a real source read of foonathan/memory's `memory_stack`, with `bumpalo`'s `Cell`-based interior-mutability pattern applied so multiple live allocations can coexist — 9 tests, real and passing on rustc 1.75, including an alignment-correctness check against the actual `unsafe` pointer arithmetic. No Miri/ASan available in this sandbox (no rustup/nightly) — verification is hand review plus tests, stated as such rather than overclaimed. `pool`/`fallback`/`segregator`/`tracking` planned, each traced to a specific real function read in foonathan/memory's source (not assumed from the library's reputation). See `docs/mid-alloc.md` |
