...
| mid-ecs | Data-oriented ECS (SoA), network sync baked in from day one | **In progress.** `World` is real — `spawn`/`despawn`/`is_alive`, a fully-wired Sparse Shell, `query`/`query2` iteration, and now a real Archetype Core with full dynamic migration (`insert_static`/`remove_static`/`get_static`/`get_static_mut`/`has_static`) — grounded in Bevy ECS's real source, cloned and read directly. 99/99 real tests across `mid-collections` + `mid-ecs`. `rayon` gated to non-wasm32 targets only, reconfirmed wasm32-clean after this addition too. `sync`/`ffi` still skeleton. Next: archetype-based query iteration, or `sync`'s replication marking |
