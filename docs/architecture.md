...
| mid-collections | Sparse set, generational arena, lock-free ring buffer, intrusive lists, hierarchical bitset, `std::hive`, FFI wrapper | **Not started, deliberately.** Design doc only — see `docs/mid-collections.md`. Not a separate build phase: pulled in piece-by-piece during the `ecs` phase as each thing is actually needed, not built upfront |
