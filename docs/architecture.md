...
| mid-ecs | Data-oriented ECS (SoA), network sync baked in from day one | **In progress.** `World` is real — `spawn`/`despawn`/`is_alive`, a fully-wired Sparse Shell (`insert`/`get`/`get_mut`/`remove`/`has`, any `T: 'static` as a component, dense `ComponentId` not `TypeId`-hashed — grounded in Bevy ECS's real source), and `query`/`query2` iteration. 73/73 real tests across `mid-collections` + `mid-ecs`. `rayon` gated to non-wasm32 targets only (`docs/mid-ecs.md`'s Platform section) — confirmed via `cargo tree`, real CI checks the wasm32 build too. `Archetype`/`sync`/`ffi` still skeleton — no dense/table storage for stable components yet. Next: the Archetype Core, or a generic tuple-based `Query<T>` once real usage patterns justify it |
