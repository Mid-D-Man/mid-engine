...
| mid-ecs | Data-oriented ECS (SoA), network sync baked in from day one | **In progress.** `World` is real — `spawn`/`despawn`/`is_alive`, generation-checked `Entity` handles, 12/12 real tests passing (mid-collections wired in as a real dependency, not just built standalone). `Archetype`/`Query`/`sync`/`ffi` still skeleton — no component storage exists yet, so a spawned entity has an identity and nothing attached to it. Next: wire `SparseSet` in as the Sparse Shell's actual storage |
